...
$inbox_count= $read_inbox_count= $unread_inbox_count=$Outbox_count=$draft_count=$publication_count="";
...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['Delete']) && isset($_POST['selectbox']))
{
	...
	if (count($sam) >0)
	{
		foreach($sam as $value)
		{
			...
			//update the id table
			$query301 = "UPDATE all_mails SET receive_delete = ? WHERE receiver_address=? AND box_type=? And Mail_id=?";
			$stmt301 = $conn->prepare($query301);
			$stmt301->execute(array('1',$_SESSION['username'],'In',$value));
	...
	<div class="row">
        <div class="col-xs-12 col-sm-12">
			...
			<div  class="col-sm-10 col-md-10 col-lg-10">
				   ...
				   style="color:yellow; font-weight:bold; font-family:'Times New Roman', Times, serif; font-size:20px;"> All INBOX <?php echo $inbox_count; ?> | READ <?php echo $read_inbox_count; ?> | UNREAD : <?php echo $unread_inbox_count; ?></center>
...
<form role="form" class="form-horizontal" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data" method="POST" name="step2">
							<input type="submit" name="Delete" value="Delete All Selected Mails"></input>
							<br/>
							<table class="table table-condensed" style="background-color:#FFFFFF;margin-top:10px">
												...
												<tbody>
													...
													$box_type="In";
													$stat="";
													$sql    = "SELECT count(*) FROM all_mails where receiver_address = '".$_SESSION['username']."' AND box_type='".$box_type."' AND receive_delete ='".$stat."'";
													...
													$status="";
													$sql = "SELECT Id,sender_name,sender_address,sender_dept,read_status,subject,body,mail_id,attached_status,date_sent,receive_delete FROM all_mails where receiver_address = '".$_SESSION['username']."' AND box_type='".$box_type."' AND receive_delete ='".$stat."' ORDER BY Id DESC LIMIT {$per_page} OFFSET {$offset} "; 
													$retval = mysql_query($sql, $conn);
													if (!$retval) {
														die('Could not get data: ' . mysql_error());
													}
													while ($row = mysql_fetch_array($retval, MYSQL_ASSOC)) 
													{
															...
															$user_link = '<a class="delpointer" href='.$j.' title='.$row['sender_address'].' data-toggle="popover">
																  '.$name.'</a>';	
															/* $user_link<button type="button" class="btn btn-default" title="Popover title"  
																  data-container="body" data-toggle="popover" data-placement="left" 
																  data-content="Some content in Popover on left">
																  Popover on left
															   </button>*/
															$checkbox='<input type="checkbox" value='.$row['mail_id'].' name="selectbox[]" ></input>';
													...
													if ($total_pages > 1)
													{
														...
														{
														echo ' <li><a href=Abu_Mail_Server_Inbox.php?page='.$previous_page.'>&laquo; </a> </li>';
														}
														 ...
														 for($i = 1; $i <= $total_pages; $i++)
														 {
															...
															//then it sset the i to be active or focused
															 ...
															 //display the page number
																echo ' <li><a href=Abu_Mail_Server_Inbox.php?page='.$i.'> '. $i .' </a></li>';
															 }
														 }
														//this is for next record		
														if ($has_next_page)
														{
															echo ' <li><a href=Abu_Mail_Server_Inbox.php?page='.$next_page.'>&raquo;</a></li> ';
														}
