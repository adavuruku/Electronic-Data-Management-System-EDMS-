...
//update Codings Begins - Name
	if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_new_name']))
	{
		...
		if(($new_name_first != FALSE) && ($new_name_last != FALSE))
		{
			...
			$stmt = $conn->prepare("UPDATE staff_information SET first_name=?,middle_name=?,last_name=?
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_new_state']))
	{
		...
		if(($cmbstate != FALSE) && ($cmblgov != FALSE) && ($cmbstate1 != "-select state-") && ($cmblgov1 !="-select local govt-"))
		{
			...
			$stmt = $conn->prepare("UPDATE staff_information SET state_origin=?,local_gov=?
											 WHERE username=? Limit 1");
...
//update Date Employed
	if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_new_emp_date']))
	{
		...
		if(($new_name_first != FALSE) && ($new_name_last != FALSE)&& ($new_name_last != FALSE))
		{
			...
			$stmt = $conn->prepare("UPDATE staff_information SET date_employed=?
											 WHERE username=? Limit 1");
...
//update Date Birth
	if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_new_date_birth']))
	{
		...
		if(($new_name_first != FALSE) && ($new_name_last != FALSE)&& ($new_name_last != FALSE))
		{
			...
			$all = $new_emp_date_year."-".$new_emp_date_month."-".$new_emp_date_day;
			//$date5 = new DateTime($all);
			$stmt = $conn->prepare("UPDATE staff_information SET dob=?
											 WHERE username=? Limit 1");
											$stmt->execute(array($all,$_SESSION['username']));
											$affected_rows = $stmt->rowCount();
											if($stmt == true) 
...
//update Email Address
		...
		if($new_email != FALSE)
		{
			...
			if($new_email != FALSE)
			{
				...
				if($row_count6 != 1) 
				{
					...
					if($row_count6 != 1) 
					{
								$stmt = $conn->prepare("UPDATE staff_information SET email=?
											 WHERE username=? Limit 1");
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_new_gender']))
	{
		...
		if($new_gender != FALSE)
		{
			...
			//$date5 = new DateTime($all);
			$stmt = $conn->prepare("UPDATE staff_information SET Gender=?
											 WHERE username=? Limit 1");
...
//update Marital Status
	if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_new_marital']))
	{
		...
		if($new_marital != FALSE)
		{
			...
			//$date5 = new DateTime($all);
			$stmt = $conn->prepare("UPDATE staff_information SET marital_status=?
											 WHERE username=? Limit 1");
...
//search for the staff if the staff did not exist log out the user it might site attack
		$query2 = "SELECT * FROM staff_information WHERE username =:username";
		...
		if($row_count2 >= 1)
		{
			...
			$staff_type = $rows3['staff_type']." In ".$rows3['course']." Programme";
			if($rows3['staff_type']=="Dean"){
				$staff_type = $rows3['staff_type']." Faculty Of Science";
			}
			if($rows3['staff_type']=="HOD"){
				$staff_type = $rows3['staff_type']." ".$rows3['department']." Department";
			}
			...
			$date_admitted = $rows3['date_employed'];
			...
			if($rows3['pic_extension']!=""){
				$_SESSION['passport'] = $paspart = "abu_file/".$user_name.$rows3['pic_extension'];
...
<script type="text/javascript" src="INDEX_FILES/js/bootstrap.js"></script>.js
				    ...
				    <div class="col-lg-12" style="width:auto; background-color:#FFFFFF; margin-top:2%; padding-top:20px; padding-left:20px;padding-bottom:5%; margin-left:0%; margin-right:0%">
						...
						<div class="col-lg-12" style="margin-bottom:0%">
							...
							<div class="col-lg-9" style="margin-bottom:0%">
									...
									<table class="table table-condensed" style="background-color:#FFFFFF;width:100%">
										<tbody>
											<tr>
												<td >Name : </td>
											...
											<tr>
												<td  style="width:45%"><input style="width:20%" type="text" value="" name="new_emp_date_day" placeholder="DD"></input>/<input style="width:20%" type="text" value="" name="new_emp_date_month" placeholder="MM"></input>/<input style="width:40%" type="text" value="" name="new_emp_date_year" placeholder="YYYY"></input></td>
											...
											<tr>
												<td  style="width:45%"><input style="width:20%" type="text" value="" name="new_birth_date_day" placeholder="DD"></input>/<input style="width:20%" type="text" value="" name="new_birth_date_month" placeholder="MM"></input>/<input style="width:40%" type="text" value="" name="new_birth_date_year" placeholder="YYYY"></input></td>
											...
											{
												echo '<tr><td colspan="2">No Record Uploaded for Staff Educational Qualification</td></tr>';
											}
											...
											<tr>
												<td colspan="3" style="color:red">Course Taken By Staff</td>
											</tr>
											<?php
											...
											if ($stmt->rowCount () >= 1)
											{
												while($rows = $stmt->fetch(PDO::FETCH_ASSOC)) 
												{
													...
													$link = "update_staff_profile.php?mataatum=".$rows['id']."&otatum=".$ota."&record=40xy";
													echo '<tr><td>'.$rows['department'].'</td><td>'.$rows['level'].'</td><td>'.$rows['course'].'</td><td ><a href='.$link.'>Remove</a></td></tr>';
												}
											}
											else
											{
												echo '<tr><td colspan="2">No Record Uploaded for Staff Current Course Taken</td></tr>';
											}
											...
											<tr>
												...
												<option value="200">100 Level</option>
