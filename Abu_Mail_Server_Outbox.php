...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['Delete']) && isset($_POST['selectbox']))
{
	...
	if (count($sam) >0)
	{
		foreach($sam as $value)
		{
			...
			//update the id table
			$query301 = "UPDATE all_mails SET sent_delete = ? WHERE sender_address=? AND box_type=? And Mail_id=?";
			$stmt301 = $conn->prepare($query301);
			$stmt301->execute(array('1',$_SESSION['username'],'Out',$value));
	...
	<div class="row">
        <div class="col-xs-12 col-sm-12">
			...
			<div  class="col-sm-10 col-md-10 col-lg-10">
				   ...
				   style="color:yellow; font-weight:bold; font-family:'Times New Roman', Times, serif; font-size:20px;"> All OUTBOX : <?php echo $Outbox_count; ?></center>
...
<form role="form" class="form-horizontal" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data" method="POST" name="step2">
							...
							<table class="table table-condensed" style="background-color:#FFFFFF;margin-top:10px">
												...
												<tbody>
													...
													$box_type="Out";
													$status="";
													$sql    = "SELECT count(*) FROM all_mails where sender_address = '".$_SESSION['username']."' AND box_type='".$box_type."' AND sent_delete ='".$status."'";
													...
													$sql = "SELECT Id,receiver_name,receiver_address,message_type,receiver_dept,subject,body,mail_id,attached_status,date_sent,sent_delete FROM all_mails where sender_address= '".$_SESSION['username']."' AND box_type='".$box_type."'  AND sent_delete ='".$status."' ORDER BY Id DESC LIMIT {$per_page} OFFSET {$offset} "; 
													$retval = mysql_query($sql, $conn);
													if (!$retval) {
														die('Could not get data: ' . mysql_error());
													}
													while ($row = mysql_fetch_array($retval, MYSQL_ASSOC)) 
													{
															...
															$add="";
															$add = substr(strip_tags($row['receiver_name']),0,30);
															$add = $add."...";
															$add2="";
															$add2 = substr(strip_tags($row['receiver_address']),0,30);
															$add2 = $add2."...";
															$user_link = '<a class="delpointer" href='.$j.' title='.$add2.'>'.$add.'</a>';	
													...
													if ($total_pages > 1)
													{
														...
														{
														echo ' <li><a href=Abu_Mail_Server_Outbox.php?page='.$previous_page.'>&laquo; </a> </li>';
														}
														 ...
														 for($i = 1; $i <= $total_pages; $i++)
														 {
															...
															//then it sset the i to be active or focused
															 ...
															 //display the page number
																echo ' <li><a href=Abu_Mail_Server_Outbox.php?page='.$i.'> '. $i .' </a></li>';
															 }
														 }
														//this is for next record		
														if ($has_next_page)
														{
															echo ' <li><a href=Abu_Mail_Server_Outbox.php?page='.$next_page.'>&raquo;</a></li> ';
														}
