...
if(isset($_GET['myfile']))
{
	...
	//replace # with underscore(_) so browser can understand it
	$file_name = str_ireplace("#", "_", $file_name);
	
	rename("Mail_Files/".$d."/".$_GET['filename'],"Mail_Files/".$d."/".$file_name);
	//$file_name = $_GET['filename'];
	header("Content-Disposition: attachment; filename=".$file_name);
	header("Content-type: application/".$ext);
	header("Pragma: no-cache"); 
	header("Expires: 0");
	readfile("Mail_Files/".$d."/".$file_name);
}
...
$query2 = "SELECT * FROM all_mails where sender_address =:sender_address AND mail_id=:mail_id AND box_type=:box_type";
$stmt2 = $conn->prepare($query2);
$stmt2->bindValue(':sender_address',$_SESSION['username'], PDO::PARAM_STR);
$stmt2->bindValue(':mail_id',$d, PDO::PARAM_STR);
$stmt2->bindValue(':box_type','Out', PDO::PARAM_STR);
$stmt2->execute();
...
if($row_count2 >= 1) 
{
	...
	$sender_address = $rows3['sender_address'];
	$sender_name = $rows3['sender_name'];
	$mail_type = $rows3['message_type'];
	$department  = $rows3['receiver_dept'];$address = $rows3['receiver_address']; $full_name= $rows3['receiver_name'];
	
	$verify_user_link = verify_exist($address);
	//retrieve pasport of receiver if is one person
	$emp="";
	$query2 = "SELECT pic_extension,username FROM student_information where username =:username";
	$stmt2 = $conn->prepare($query2);
	$stmt2->bindValue(':username',$address, PDO::PARAM_STR);
	$stmt2->execute();
	$rows3 = $stmt2->fetch(PDO::FETCH_ASSOC);
	$row_count2 = $stmt2->rowCount();
	if($row_count2 >= 1) 
	{
		...
		{
			$passport ="";
			$paspart = "abu_file/".$address.$rows3['pic_extension'];
...
if ($p == "")
{
		$passport ='The Mail was Send To All the Users Bellow <br>';
		$department =""; 
		$full_name= "";
		$address = "<br> The Address are : <br>".$address;
		$verify_user_link ="";
}

if ($mail_type !="Normal_Mail")
{
	$passport ='The Mail was Send To Every User That Fall in to the Group<br> '. $mail_type;
	$department =""; 
	$full_name= "";
	$address = "<br> The Address are : <br>".$address;
	$verify_user_link ="";
...
$verify_user_link = "view_student_profile.php?username=".$address;
$verify_user_link ='<a href='.$verify_user_link.'>View Full Profile </a>'; 
}
if($verify_user_link == "staff"){
$verify_user_link = "view_staff_profile.php?username=".$address;
$verify_user_link ='<a href='.$verify_user_link.'>View Full Profile </a>'; 
}
			...
			<div  class="col-sm-10 col-md-10 col-lg-10">
				   ...
				   <center><span style="color:#FFFFFF; font-weight:bold; font-family:'Times New Roman', Times, serif; font-size:16px;"> <?php echo $_SESSION['username']; ?> - You are Currently Log In</span></center>
				    <div class="col-lg-12" style="width:auto; background-color:#FFFFFF; margin-top:2%; padding-top:20px; padding-left:20px;padding-bottom:5%; margin-left:0%; margin-right:0%">
						...
						<div class="col-lg-9" style="margin-bottom:0%">
								<div  class="col-lg-12" style="width:100%; padding-top:10px; padding-left:5px; padding-bottom:10px; background-color:grey;margin-bottom:1%">
									<div  class="col-lg-7">
										...
										<div  class="col-lg-9">
											<?php echo $sender_name."<br>".$sender_address;?>
										</div>
									</div>
						...
						<div  class="col-lg-3" style="overflow:scroll;height:400px">
						<!-- holds image and other details of user -->
						<?php echo $passport;?>
						<p><?php echo $full_name;?></p>
						<p><?php echo $address;?></p>
						<p><?php echo $department;?><p>
						<p><?php echo $verify_user_link;?></p>
